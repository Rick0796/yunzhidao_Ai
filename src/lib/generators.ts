...
export async function runDraftGeneration(
  ...
  const selectedParts = [
    `已选钩子：${hook.text}`,
    !isViral && `已选骨架：${skeleton.name}（${skeleton.steps.map(s => s.name).join(" → ")}）`,
    !isViral && "内容策略：",
    ...(!isViral ? formatTaskStrategyLines(strategy) : []),
    !isViral && "骨架执行单：",
    ...(!isViral ? skeletonExecutionLines : []),
    viralParagraphGuide,
  ...
  return generateJson({
    ...
    instruction: [
      ...
      "2. 不能在钩子前加任何铺垫或解释。",
      ...(isViral ? [
        "3. 仿写任务：正文严格按原文段落顺序逐段改写，不能重新排列段落，不能合并段落，不能跳段。",
        "4. 每一段只做同义换词和句式微调，原文的爆点、数字、判断句、三点式结构必须保留。",
        "5. 不能用骨架逻辑重新组织内容，骨架仅作参考，原文段落顺序优先。",
      ] : [
      ...
      task.entryType === "viral"
        ? "【仿写爆款】\n- 严格按原文的段落结构和推进顺序写，段落数必须与原文一致\n- 总字数必须在原文的85%到115%之间，不能大幅缩短\n- 原文的爆点、判决句、反问句、数字、后果和三点式结构，必须全部保留\n- 改写只做轻微调词、去重、换表达，不要改骨架，不要新增新的论证层"
      : task.entryType === "hotspot"
      ...
      ...(isViral ? [
        ...
        "- 版本2（换皮版）：只重写第一句钩子，让开头更炸，中段和结尾贴原文走，其余只做轻微调词",
        ...
        "- 严格要求：每个版本都必须包含原文所有核心爆点、数字、判断句，不能删段、不能合并段落",
        "- 严格要求：每个版本独立生成，不能复制粘贴其他版本再微调",
      ] : [
        "- 版本1（标准版）：按照骨架标准展开，情绪中性，适合大众",
        "- 版本2（激进版）：情绪更强烈，判断更犀利，用词更冲击；换掉版本1中所有平缓句，改用强判决和极端对比",
        "- 版本3（判决版）：像下判决书一样，语气坚定，结论明确；每段都以判决句收尾，禁止出现疑问句",
        "- 版本4（口语版）：更像聊天，多用短句、反问、停顿、叹词，接地气；句子平均不超过12字",
        "- 版本5（老板版）：像老板自己讲话，有经验感、有决策感、有格局感；多用第一人称，带个人经历感",
        "- 严格要求：5个版本的措辞、句式、情绪力度必须明显不同，不能只改几个词，要让读者一眼看出差异",
        "- 严格要求：每个版本独立生成，不能复制粘贴其他版本再微调",
      ]),
      "",
      buildEntryWorkflowInstruction(task.entryType)
    ...
    schemaHint: JSON.stringify(
      {
        items: [
          {
            id: "string",
            versionName: isViral ? "最近原文版 / 换皮版 / 精炼版 / 口语强化版 / 人群切换版" : "标准版 / 激进版 / 判决版 / 口语版 / 老板版",
            title: "string",
            coverLine: "string",
            script: "string",
