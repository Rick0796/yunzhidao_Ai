...
function buildViralRewriteInstruction(
  task: TaskForm,
  hook: HookItem,
  meat: MeatItem | null,
  cta: CtaItem
...
export async function runDraftGeneration(
  ...
  return generateJson({
    ...
    instruction: [
      ...
      "2. 热点任务要用热点本身做推进，不要空泛说教。",
      "3. 老板故事要基于经历口述，不要编出额外桥段。",
      "4. 不能只让第一句和最后一句贴题，中段每一步也必须继续使用用户给的参考内容、事实、判断和方向，不要写成通用模板句式。",
      "5. 中段前两段优先复用或改写素材里的事实、对象、动作、时间点、平台名、人物名，不要一上来就写抽象判断。",
      "6. 如果素材里有2到4个连续信息点，就按那个顺序往下推，不要把所有判断都塞到最后。",
      "7. 中段必须持续给内容回报：事实承接、判断升级、原因解释、概念桥、结果落点，至少要占到其中3层以上。",
      `8. 以下跳转禁止出现：${strategy.forbiddenJumps.join("；")}`,
      "",
      "=== 字数与节奏 ===",
