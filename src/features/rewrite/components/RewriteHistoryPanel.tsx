...
export default function RewriteHistoryPanel({ history, onLoad, onDelete, onClear }: RewriteHistoryPanelProps) {
  return (
    <div className="animate-fade-in space-y-3 rounded-3xl border border-white/10 bg-[#0A0F1F]/95 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl">
      <div className="flex items-center justify-between gap-3">
        <span className="text-sm font-semibold text-white">{TEXT.title}</span>
        {history.length > 0 ? (
          <button type="button" onClick={onClear} className="text-xs text-red-400 transition-colors hover:text-red-300">
            {TEXT.clear}
          </button>
        ) : null}
      </div>
      {history.length === 0 ? <p className="py-8 text-center text-sm text-slate-500">{TEXT.empty}</p> : null}
      {history.length > 0 ? (
        <div className="max-h-[420px] space-y-2 overflow-y-auto pr-1">
          {history.map((item) => (
            <div
              key={item.id}
              className="group flex cursor-pointer items-start gap-3 rounded-2xl border border-white/5 bg-white/5 p-4 text-left transition-all hover:bg-white/10"
              onClick={() => onLoad(item)}
              onKeyDown={(event) => {
